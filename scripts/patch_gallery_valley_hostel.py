...
# Define the new objects for Day 11 to Day 15
days_11_to_15 = [
  {
    "dayNum": 11,
    "day": "Day 11",
...
# Read js/gallery-data.js
with open('js/gallery-data.js', 'r', encoding='utf-8') as f:
    content = f.read()

# Parse galleryData
prefix = 'window.galleryData = '
if prefix in content:
    json_str = content[content.index(prefix) + len(prefix):].rstrip().rstrip(';')
    data = json.loads(json_str)
    
    # Filter out old days 11 to 15
    new_data = [d for d in data if d.get('dayNum') not in [11, 12, 13, 14, 15]]
    ...
    for i, d in enumerate(days_11_to_15):
        new_data.insert(insert_idx + i, d)
        
    # Write back
    new_content = '// Schengen Tour 2026-2027 - Complete Verified Visual Gallery Data\n// Granular, multi-photo photography (At least 5 photos per genuine sightseeing landmark)\n\nwindow.galleryData = ' + json.dumps(new_data, indent=2, ensure_ascii=False) + ';\n'
    with open('js/gallery-data.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("✅ js/gallery-data.js updated with Days 11-15 Valley Hostel itinerary!")
else:
    print("❌ Could not find window.galleryData in js/gallery-data.js")

# Update generate_pdf.py Days 11-15
with open('generate_pdf.py', 'r', encoding='utf-8') as f:
    pdf_code = f.read()

old_pdf_chunk = '''        ("Day 11<br/>25 Dec 2026", "Strasbourg ➔<br/>Bern Base (CH)", "<b>Morning (08:30 – 12:00):</b><br/>&nbsp;&nbsp;• Transit Strasbourg to Bern Hbf<br/>&nbsp;&nbsp;• Alpenblick CoLiving check-in<br/><b>Afternoon & Evening (13:00 – 21:00):</b><br/>&nbsp;&nbsp;• Bern UNESCO Old Town & 6 km Lauben arcades<br/>&nbsp;&nbsp;• Zytglogge & Christmas fondue dinner", "Alpenblick CoLiving, Bern<br/>(Night 1 of 4 · Confirmed)"),
        ("Day 12<br/>26 Dec 2026", "Grindelwald &<br/>Lake Brienz (CH)", "<b>Morning (08:30 – 13:00):</b><br/>&nbsp;&nbsp;• SBB Train: Bern ➔ Grindelwald (1h 34m)<br/>&nbsp;&nbsp;• First Gondola to Mount First (2,168m)<br/>&nbsp;&nbsp;• First Cliff Walk by Tissot (Eiger views)<br/><b>Afternoon (13:30 – 17:30):</b><br/>&nbsp;&nbsp;• PostBus 103 to Lake Brienz<br/>&nbsp;&nbsp;• Iseltwald Landing Stage (CLOY Pier)<br/><b>Evening (18:00 – 21:00):</b><br/>&nbsp;&nbsp;• Return train to Bern Base", "Alpenblick CoLiving, Bern<br/>(Night 2 of 4 · Confirmed)"),
        ("Day 13<br/>27 Dec 2026", "Lauterbrunnen, Mürren<br/>& Blausee (CH)", "<b>Morning (08:30 – 12:30):</b><br/>&nbsp;&nbsp;• SBB Train: Bern ➔ Lauterbrunnen (1h 15m)<br/>&nbsp;&nbsp;• Staubbach Falls cascading 297m<br/>&nbsp;&nbsp;• BLM Cable Car & Mountain Rail to Mürren (1,638m)<br/><b>Afternoon & Twilight (13:30 – 19:30):</b><br/>&nbsp;&nbsp;• SBB + Bus 230 to Blausee Nature Park<br/>&nbsp;&nbsp;• Path of the 1,000 Lights lantern walk<br/><b>Evening (20:00 – 21:30):</b><br/>&nbsp;&nbsp;• Return train to Bern Base", "Alpenblick CoLiving, Bern<br/>(Night 3 of 4 · Confirmed)"),
        ("Day 14<br/>28 Dec 2026", "Lake Thun, Sigriswil<br/>& Bern (CH)", "<b>Morning (09:30 – 12:30):</b><br/>&nbsp;&nbsp;• SBB InterCity: Bern ➔ Spiez on Lake Thun (30m)<br/>&nbsp;&nbsp;• Spiez Castle waterfront & bay promenade<br/><b>Afternoon (13:00 – 15:30):</b><br/>&nbsp;&nbsp;• STI Bus to Sigriswil Suspension Bridge (182m high)<br/><b>Sunset & Evening (16:00 – 21:00):</b><br/>&nbsp;&nbsp;• Bern Rosengarten sunset panorama over Aare river<br/>&nbsp;&nbsp;• Cosy Swiss bistro dinner · Night 4 in Bern", "Alpenblick CoLiving, Bern<br/>(Night 4 of 4 · Confirmed)"),'''

new_pdf_chunk = '''        ("Day 11<br/>25 Dec 2026", "Strasbourg ➔<br/>Lauterbrunnen (CH)", "<b>Morning (04:05 – 11:30):</b><br/>&nbsp;&nbsp;• FlixBus N846 to Lucerne (04:05–07:10)<br/>&nbsp;&nbsp;• Zentralbahn Brünig Pass to Interlaken & Lauterbrunnen<br/><b>Afternoon & Evening:</b><br/>&nbsp;&nbsp;• Valley Hostel check-in & Staubbach Falls<br/>&nbsp;&nbsp;• Communal kitchen Christmas holiday dinner", "Valley Hostel, Lauterbrunnen<br/>(Night 1 of 4 · Confirmed)"),
        ("Day 12<br/>26 Dec 2026", "Grindelwald &<br/>Kleine Scheidegg (CH)", "<b>Morning (08:30 – 13:00):</b><br/>&nbsp;&nbsp;• Mountain rail to Grindelwald-First<br/>&nbsp;&nbsp;• First Cliff Walk by Tissot (Eiger views)<br/><b>Afternoon & Evening:</b><br/>&nbsp;&nbsp;• Historic cogwheel to Kleine Scheidegg (CLOY pass)<br/>&nbsp;&nbsp;• Cogwheel train back to Lauterbrunnen Valley", "Valley Hostel, Lauterbrunnen<br/>(Night 2 of 4)"),
        ("Day 13<br/>27 Dec 2026", "Lake Brienz &<br/>Iseltwald (CH)", "<b>Morning (09:00 – 12:00):</b><br/>&nbsp;&nbsp;• Train to Interlaken Ost + PostBus 103<br/>&nbsp;&nbsp;• Lake Brienz turquoise shoreline<br/><b>Afternoon & Evening:</b><br/>&nbsp;&nbsp;• Iseltwald Pier (CLOY piano scene)<br/>&nbsp;&nbsp;• Traditional cheese fondue in Interlaken", "Valley Hostel, Lauterbrunnen<br/>(Night 3 of 4)"),
        ("Day 14<br/>28 Dec 2026", "Mürren &<br/>Schilthorn (CH)", "<b>Daytime (09:00 – 16:00):</b><br/>&nbsp;&nbsp;• BLM Cable Car to car-free Mürren (1,638m)<br/>&nbsp;&nbsp;• Schilthorn Piz Gloria 360° summit (2,970m)<br/><b>Evening:</b><br/>&nbsp;&nbsp;• Valley Hostel packing & mobile ticket prep", "Valley Hostel, Lauterbrunnen<br/>(Night 4 of 4)"),'''

if old_pdf_chunk in pdf_code:
    pdf_code = pdf_code.replace(old_pdf_chunk, new_pdf_chunk)
    with open('generate_pdf.py', 'w', encoding='utf-8') as f:
        f.write(pdf_code)
    print("✅ generate_pdf.py updated!")
else:
    print("ℹ️ Check generate_pdf.py chunk")
